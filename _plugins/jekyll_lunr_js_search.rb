...
module Jekyll
  ...
  class Indexer < Generator
    ...
    def generate(site)
      ...
      items.each do |item|
        entry = SearchEntry.new(item, content_renderer)
...
module Jekyll
  ...
  class SearchEntry
    attr_reader :title, :url, :date, :categories, :body, :question, :excerpt

    def initialize(item, renderer)
      @title, @url, @question = extract_title_url_and_question(item)
      @body = renderer.render(item)
      @excerpt = @body.split(/\s+/, 30+1)[0...30].join(' ') << "..."
      @date, @categories = item.is_a?(Jekyll::Post) ? [post.date, post.categories] : [nil, []]
    end

    def extract_title_url_and_question(item)
      data = item.to_liquid
      [ data['title'], data['url'], data['question'] ]
    end
